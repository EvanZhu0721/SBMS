...
$maintenancePipeTransport = Read-Utf8 'maintenance-service/MaintenancePipeTransportContracts.cs'
...
$maintenancePipeTransportTests = Read-Utf8 'tests/MaintenancePipeTransportContractTests.cs'
...
Assert-True ($maintenanceBuild.Contains('MaintenancePipeTransportContracts.cs') -and -not $setupBuild.Contains('MaintenancePipeTransportContracts')) 'Only the maintenance service build may compile the pure pipe transport slot.'
Assert-True ($maintenanceContractTest.Contains('tests\MaintenancePipeTransportContractTests.cs') -and $maintenanceRuntimeTests.Contains('MaintenancePipeTransportContractTests.Run')) 'The maintenance runtime oracle must compile and execute the pipe transport state-machine contracts.'
Assert-True ($maintenancePipeTransport.Contains('MaxRequestFrameLength') -and $maintenancePipeTransport.Contains('CreateReadOperation(') -and $maintenancePipeTransport.Contains('request must fit one message read') -and -not $maintenancePipeTransport.Contains('CreateNamedPipe') -and -not $maintenancePipeTransport.Contains('ConnectNamedPipe') -and -not $maintenancePipeTransport.Contains('FlushFileBuffers')) 'The transport slot must remain a pure injectable single-message bounded-read contract without native hosting or flush semantics.'
Assert-True ($maintenancePipeTransport.Contains('MaintenanceConnectedRequestExecutor') -and $maintenancePipeTransport.Contains('sequencer.Execute(') -and $maintenancePipeTransport.Contains('MaintenancePipeSlotState.Parsing') -and $maintenancePipeTransportTests.Contains('ReadAndAuthorizationOrderingIsStrict') -and $maintenancePipeTransportTests.Contains('CancellationAlwaysDrainsExactOperation') -and $maintenancePipeTransportTests.Contains('LostResponseWriteReplaysWithoutMutation')) 'Transport parsing must stay behind typed authorization, with exact cancellation drain and replay coverage.'
Assert-True ($maintenancePipeTransport.Contains('Interlocked.CompareExchange(') -and $maintenancePipeTransport.Contains('Interlocked.Exchange(ref running, 0)') -and $maintenancePipeTransport.Contains('if (poisoned)') -and $maintenancePipeTransportTests.Contains('ConcurrencyAndPoisonAreFailClosed')) 'A transport slot must reject concurrent entry atomically and poison reuse after uncertain teardown.'
Assert-True ($maintenancePipeTransport.Contains('out Exception cancellationFailure') -and $maintenancePipeTransport.Contains('drained = operation.Drain()') -and $maintenancePipeTransport.Contains('after exact drain') -and $maintenancePipeTransportTests.Contains('CancelFailure')) 'Cancel faults and invalid cancel dispositions must still exact-drain the operation before disposal.'
Assert-True ($maintenancePipeTransport.Contains('MaintenancePipeOperationIdentity') -and $maintenancePipeTransport.Contains('Object.ReferenceEquals(') -and $maintenancePipeTransport.Contains('expectedOperationGeneration') -and $maintenancePipeTransport.Contains('enum MaintenancePipeTransferMode') -and $maintenancePipeTransport.Contains('MessageModeSingleMessage') -and -not $maintenancePipeTransport.Contains('DateTime deadlineUtc')) 'Pipe operations must bind opaque per-operation identities and monotonic operation generations under an explicit single-message transport contract and relative timeout.'
Assert-True ($maintenancePipeTransport.Contains('FailStopUncertainLifetime(') -and $maintenancePipeTransport.Contains('terminator.Terminate(reason)') -and $maintenancePipeTransport.Contains('Environment.FailFast(reason, failStopCause)') -and $maintenancePipeTransportTests.Contains('UncertainLifetimeAlwaysFailStops')) 'Unknown pipe ownership after Begin, drain, disposal, factory, or connection cleanup must terminate with a FailFast fallback.'
Assert-True ($maintenanceContracts.Contains('class MaintenanceCommittedResponse') -and $maintenanceContracts.Contains('IsCommitAuthority(authority)') -and $maintenanceContracts.Contains('ExecuteCommitted(') -and $maintenanceClientAuthorization.Contains('MaintenanceCommittedResponse Dispatch(') -and $maintenanceClientAuthorization.Contains('response.ValidateForCommand(command)') -and -not $maintenancePipeTransport.Contains('interface IMaintenanceConnectedRequestExecutor')) 'Only the sealed sequencer adapter backed by replay-issued typed commit evidence may feed the transport slot.'
