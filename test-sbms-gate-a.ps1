...
try {
    ...
    $session = [pscustomobject][ordered]@{
        ...
        computerName='TESTHOST'; lastBootUtc='2026-07-14T00:00:00Z'; clientAddress='192.0.2.20'
    }
    $sessionCapture = { $session }.GetNewClosure()
    $proof = Confirm-SBMSGateARemoteHealth -RunId $runId -RunDirectory $root -Challenge $first.challenge -CaptureSession $sessionCapture
    Assert-Throws { Confirm-SBMSGateARemoteHealth -RunId $runId -RunDirectory $root -Challenge $first.challenge -CaptureSession $sessionCapture } 'already consumed' 'SSH proof replay must be rejected.'
