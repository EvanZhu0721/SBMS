...
- 点击物理镜像后捕获鼠标并转发到虚拟桌面：支持相对移动、左/右/中/X1/X2 键、垂直/水平滚轮，以及可见的高对比度软件指针标记。按 F8 释放捕获。
...
- 镜像端把指针可见性和虚拟桌面位置显式合成进已租用的 BGRA 帧，不再假定 IDD 交换链画面天然包含光标。当前标记使用固定形状，不复现 I-beam、缩放、动画或应用自定义光标。
...
输出为 `target\installer\SBMS-Setup-0.2.10-x64.exe`。构建脚本还会用同一测试证书签名两个 Rust 可执行文件和最终安装器，验证全部签名，并打印安装包 SHA-256。

## 安装与使用

运行 `SBMS-Setup-0.2.10-x64.exe` 并批准 UAC。安装器会安装/升级驱动、更新安装器拥有的文件、仅删除明确列出的旧版遗留文件、为当前交互式安装账户注册提权登录任务并启动托盘；它不会清空安装目录中的任意文件。固定 Inno `AppId` 让后续 0.2.x 包执行原位升级。
