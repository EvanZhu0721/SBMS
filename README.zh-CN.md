...
3. IDD 通过两个共享 BGRA 槽发布每个 IddCx 交换链表面；渲染器跟不上时丢帧，而不是阻塞 IddCx。
4. 只有当 Rust 将第一帧有效共享画面绘制到选定的物理目标后，启动才算成功。
5. 停止时先拆除镜像，再关闭唯一拥有的 `HSWDEVICE`。
6. 关闭该句柄会使当前设备节点变为不在场；Windows 仍可能保留它的历史设备记录。
...
- 固定为 1920×1080@60、BGRA 的虚拟显示器。
- 通过活动 DisplayConfig `monitorDevicePath` 明确选择目标。
- 首帧确认、有界停止、连续五次启停，以及并发会话拒绝。
- `--version`、`list`、`create` 和 `map`；其中 `create` 只测试原始设备生命周期。

v2 帧通道使用两个槽。Rust 租用已发布的槽并直接交给 `StretchDIBits`；驱动写入另一个槽，若无法写入则丢弃该帧。Rust 不再进行整帧复制。缩放使用性能优先的 `COLORONCOLOR`，因此在非 1920×1080 目标上，画面会比旧版半色调输出更锐利，也更容易出现像素感。这并非零拷贝：D3D11 暂存资源回读、一次从驱动到共享内存的复制以及 GDI 输出依然存在。
...
从 `physical` 行复制完整的 `id=`。它是 `monitorDevicePath`，不是 `\\.\DISPLAYn`；`map` 启动时，该目标必须仍处于活动状态。只有在 Windows 暴露虚拟显示源，并且 Rust 绘制出第一帧有效画面后，命令才会打印 `running=`。按 Enter 停止。需要限定运行时长的无人值守用法：
